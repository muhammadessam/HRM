...
@section('javascript')
    ...
    $('document').ready(function(){
      ...
      $(document).on("click", "#br_generate", function()
      {
        $('#loading').modal('show');
        $.ajax({
      						 url: "{{url('admin/ajax/barcode/generate')}}",
      						 method: 'get',
    ...
    $(document).on("click", "#rq_generate", function()
    {
      $('#loading').modal('show');
      $.ajax({
                 url: "{{url('admin/ajax/rqcode/generate')}}",
                 method: 'get',
    ...
    $(document).on("click", "#br_print", function()
    {
      ...
      $('#loading').modal('show');
      $.ajax({
                 url: "{{url('admin/ajax/barcode/download')}}",
                 method: 'get',
                 data:{code:$('#code_bare_number').val(),staff:$('#staff').find(":selected").val(),},
                 success: function(data)
                 {
                   ...
                   bootoast.toast({
                    ...
                    window.open("{{url('admin/ajax/barcode/download?')}}code="+$('#code_bare_number').val()+"&staff="+$('#staff').find(":selected").val(), '_blank');
                 }
            });
  });
