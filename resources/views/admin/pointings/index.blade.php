...
@section('content')
    ...
    <h3 class="page-title">@lang('quickadmin.pointings.title')</h3>
    {{-- hossam --}}
                ...
                @if (count($pointings) > 0)
                    @foreach ($pointings as $pointing)
                        <tr data-entry-id="{{ $pointing->id }}" data-color="{{ $pointing->color }}">
                            ...
                            <td>
                                @can('pointing_edit')
                                    <a href="{{ route('admin.pointings.edit',[$pointing->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                @endcan
                            </td>
                        </tr>
                    @endforeach
